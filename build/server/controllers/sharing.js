...
var Album, Client, NotificationsHelper, Sharing, UserSharing, answer, async, cache, clearance, clearanceCtl, clientDS, cozydb, create, createShare, createUserSharing, getCredentials, getDisplayName, getUser, getUserPassword, localization, localizationManager, log, randomString, ref, updateOrCreate;
...
answer = function(sharing, callback) {
  ...
  if (sharing.accepted === true) {
    return getUser(sharing.url, function(err, user) {
      return getCredentials(user, sharing, function(err, password) {
...
getCredentials = function(user, sharing, callback) {
  if (user != null) {
    return getUserPassword(user.id, function(err, password) {
    ...
    return createUserSharing(user, function(err, access) {
      console.log('user created' + JSON.stringify(access));
      return callback(err, access.password);
    });
  }
};
...
create = function(attributes, callback) {
  ...
  return Sharing.create(data, function(err, sharing) {
    return callback(err, sharing);
  });
};
